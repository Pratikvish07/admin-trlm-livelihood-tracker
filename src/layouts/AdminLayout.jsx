...
const links = [
  ...
  { to: '/admin/user-management', label: 'User Management', permission: ['manage_district', 'manage_block', 'full_access'] },
  { to: '/admin/admin-approval', label: 'Admin Approval', permission: ['approve_users'] },
  { to: '/admin/all-admin-staff', label: 'All Admin & Staff', permission: ['approve_users', 'full_access'] },
...
export default function AdminLayout() {
  ...
  const filteredLinks = links.filter((link) => {
    ...
    return link.permission.some((permission) => user.permissions.includes(permission));
  });
    ...
    <>
      ...
      <div className="relative mb-5">
        ...
        <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 px-3 py-3">
          ...
          <p className="mt-1 text-[11px] uppercase tracking-[0.18em] text-slate-300">{user?.apiRole || user?.role}</p>
          {user?.district ? <p className="mt-2 text-[11px] text-slate-300">District: {user.district}</p> : null}
  ...
  const currentLabel =
    filteredLinks.find((link) => location.pathname.startsWith(link.to))?.label || 'Dashboard';
