...
export default function UserManagementPage() {
  ...
  const load = async (nextPage = pageNumber, nextSize = pageSize, options = {}) => {
    ...
    try {
      ...
      if ((result.totalPages || 1) > nextPage && !cacheRef.current.has(nextCacheKey)) {
        api.getShgMembers(nextPage + 1, nextSize).then((prefetched) => {
  ...
  const districtOptions = useMemo(
    () =>
      Array.from(
        new Set(rows.map((row) => readField(row, fieldAliases.district)).filter(Boolean))
  ...
  const villageOptions = useMemo(
    () =>
      Array.from(
        new Set(rows.map((row) => readField(row, fieldAliases.village)).filter(Boolean))
      ).sort((a, b) => a.localeCompare(b)),
    [rows]
  );
  ...
  const processedRows = useMemo(() => {
    const filtered = rows.filter((row) => {
      ...
      if (q && !`${memberName} ${lokosId} ${district} ${village} ${mobile} ${shgName}`.includes(q)) return false;
  ...
  const displayColumns = useMemo(() => {
    ...
    const extraKeys = Object.keys(processedRows[0]).filter(
      (key) => !Object.values(fieldAliases).flat().includes(key)
    );
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="portal-subtitle mt-2 max-w-3xl">
              View uploaded SHG member data with filters, sorting, and pagination.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="mt-5 space-y-3">
          <div className="grid gap-3 sm:grid-cols-2 2xl:grid-cols-4">
            <input
              ...
              placeholder="Search member, SHG, village, mobile"
              value={search}
          ...
          {showFilters ? (
            <div className="grid gap-3 sm:grid-cols-2 2xl:grid-cols-4">
              ...
              <input
                className="rounded-2xl border border-slate-300 bg-white p-3"
                placeholder="Filter by Lokos ID"
                value={lokosFilter}
                onChange={(e) => setLokosFilter(e.target.value)}
              />
            </div>
          ) : null}
      ...
      <div className="portal-card overflow-hidden">
        <div className="flex flex-col gap-3 border-b border-slate-200 bg-slate-50/70 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2">
            <button
              className="portal-btn-outline disabled:opacity-50"
        ...
        {!loading && !error && processedRows.length === 0 ? (
          <p className="px-4 py-4 text-slate-500">No SHG members found for the current filters.</p>
        ) : null}
      </div>
    </div>
  );
