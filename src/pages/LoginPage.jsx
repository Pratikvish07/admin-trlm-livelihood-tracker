            ...
            <div className="relative">
              ...
              <div className="mt-5 grid max-w-lg gap-2.5 sm:grid-cols-2">
                {[
                  ...
                  <div key={k} className="rounded-2xl border border-white/20 bg-white/10 p-3.5">
            ...
            <div className="relative mx-auto w-full max-w-[460px] overflow-hidden rounded-[32px] border border-slate-200/80 bg-[linear-gradient(180deg,#ffffff_0%,#f7fbff_100%)] p-5 shadow-[0_24px_60px_rgba(15,32,70,0.16)] md:p-6">
              ...
              <div className="relative">
                ...
                <div className="mt-5 grid grid-cols-2 rounded-2xl bg-slate-100 p-1.5">
                  <button type="button" className={`rounded-2xl px-4 py-3 text-sm font-bold transition ${activeTab === 'login' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`} onClick={() => setActiveTab('login')}>Login</button>
                  <button type="button" className={`rounded-2xl px-4 py-3 text-sm font-bold transition ${activeTab === 'signup' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`} onClick={() => setActiveTab('signup')}>Sign Up</button>
                </div>

                {signupMessage ? <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">{signupMessage}</div> : null}
                    ...
                    <div className="mt-6 space-y-5">
                      <div>
                        ...
                        <div className="mt-2 flex items-center rounded-2xl border border-slate-200 bg-white px-4 py-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)] transition focus-within:border-cyan-400 focus-within:ring-4 focus-within:ring-cyan-100">
                          ...
                          <input className="w-full bg-transparent py-3 text-lg uppercase text-slate-800 outline-none placeholder:text-slate-300" value={userId} placeholder="Enter user ID" onChange={(e) => { if (error) dispatch(clearAuthError()); setUserId(e.target.value); }} />
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="mt-2 flex items-center rounded-2xl border border-slate-200 bg-white px-4 py-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)] transition focus-within:border-cyan-400 focus-within:ring-4 focus-within:ring-cyan-100">
                          <span className="pr-3 text-lg text-slate-400">*</span>
                          <input type={showPassword ? 'text' : 'password'} className="w-full bg-transparent py-3 text-lg text-slate-800 outline-none placeholder:text-slate-300" value={password} placeholder="Enter password" onChange={(e) => { if (error) dispatch(clearAuthError()); setPassword(e.target.value); }} />
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <div>
                        ...
                        <select className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none focus:border-cyan-400 focus:ring-4 focus:ring-cyan-100" value={signupForm.role} onChange={(e) => updateSignupField('role', e.target.value)}>
                          {signupRoleOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
                        </select>
                      </div>
                      <div>
